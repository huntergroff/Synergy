...
import { SimpleClassTables } from "./ClassTables.jsx";
  ...
  if (event) {
    return (
      <div className="eventbox">
        ...
        {event.IMAGE_OPTIONAL ? (
          <img src={event.IMAGE_OPTIONAL} alt="New Event"></img>
        ) : (
          <></>
        )}
        <div>
          ...
          <p className="datetime">
            {event.DATE_TIME ? event.DATE_TIME : ""} <br />{" "}
            {event.LOCATION_OPTIONAL ? `at ${event.LOCATION_OPTIONAL}` : ""}
          </p>
          {event.DESCRIPTION ? (
          ...
          {event.LINK_OPTIONAL && event.LINK_TEXT_OPTIONAL ? (
            <a
              href={event.LINK_OPTIONAL}
              target="_blank"
              rel="noopener noreferrer"
            >
              {event.LINK_TEXT_OPTIONAL}
            </a>
          ) : (
            <></>
          )}
...
const Events = () => {
  ...
  useEffect(() => {
    Papa.parse(
      "https://docs.google.com/spreadsheets/d/e/2PACX-1vQvCf3uHNT26Wb9ApDfYVSPbUIBsX7fjqejut1LCRptL8TET9DDmPElpL9IsJgosqT1aMUscfVIrzjC/pub?gid=62357513&single=true&output=csv",
      {
        download: true,
        header: true,
  ...
  return (
    <div className="events container">
      <div className="events-header">
        ...
        <h2>
          Check back here for a regularly updated list of upcoming events at
